...
public class NWayOverlapTask extends SwingWorker<Void, Void> {
	...
	public NWayOverlapTask(MainMatCalcWindow window, 
			List<Path> files,
	...
	private void overlap() {
		...
		for (int i = 0; i < mAnnotations.size(); ++i) {
			...
			for (Chromosome chr : gs) {
				for (Annotation an : gs.getFeatures(chr)) {
					...
					while(!exhausted) {
						Map<Path, Set<GenomicRegion>> overlaps = 
								DefaultTreeMap.create(new TreeSetCreator<GenomicRegion>());
						...
						GenomicRegion consensusRegion = region; //GenomicRegion.create(chr, region.getStart(), region.getEnd())
				...
				for (int i = 0; i < mAnnotations.size(); ++i) {
					...
					if (locationCoreMap.get(chr).get(overlapRegion).containsKey(file)) {
						...
						List<GenomicRegion> locations = 
								CollectionUtils.sort(locationCoreMap.get(chr).get(overlapRegion).get(file));
		...
		if (mVenn) {
			if (mFiles.size() == 2) {
				MainVennWindow vennWindow = new MainVennWindow(group1, 
						group2, 
						CircleStyle.UNIFORM);
