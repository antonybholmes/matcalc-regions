...
/**
 * Displays a sample as a two line node with title and some
 * basic info. Headers are displayed in bold
 *
...
public class EnhancerTreeNodeRenderer extends ModernTreeNodeRenderer {
	...
	public final void drawBackground(Graphics2D g2) {
		Rectangle r = new Rectangle(mRect.getX(), 
				mRect.getY(), 
				mRect.getW(), 
				mRect.getH());
	...
	public ModernTreeNodeRenderer getRenderer(Tree<?> tree,
		...
		if (node.isParent()) {
			...
			if (sample != null) {
				text1 = sample.getName(); // + " (" + sample.getOrganism().getScientificName() + ")";
