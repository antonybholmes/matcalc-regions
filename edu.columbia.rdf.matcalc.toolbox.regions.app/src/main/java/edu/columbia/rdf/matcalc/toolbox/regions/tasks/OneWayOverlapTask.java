...
/**
 * Performs a one way overlap between two files. The structure of the
 * first file is preserved and we annotate whether features of the second
 * file overlap those in the first.
 */
public class OneWayOverlapTask extends SwingWorker<Void, Void> {
	 ...
	 */
	public OneWayOverlapTask(MainMatCalcWindow window,
			List<Path> files,
	...
	private void overlap() throws Exception {
		...
		for (int i = 0; i < matrix.getRowCount(); ++i) {
			...
			List<GappedSearchFeatures<Annotation>> results = 
					gappedSearch.getFeatures(region);
