...
public class RegionsModule extends CalcModule implements ModernClickListener {
	...
	private class EnhancerTask extends SwingWorker<Void, Void> {
		...
		public EnhancerTask(Map<String, 
			...
			for (int i = 0; i < model.getRows(); ++i) {
				...
				for (int j = 0; j < annotations.size(); ++j) {
					...
					List<GappedSearchFeatures<Annotation>> results = 
							mGappedSearch.get(type).getFeatures(region);
	...
	private class OverlapTask extends SwingWorker<Void, Void> {
		...
		private GappedSearch<Annotation> mGappedSearch;
		private XlsxTableModel mNewModel;
		private MessageDialogTaskGlassPane mSearchScreen;
		private Path mFile2;
		private boolean mAddBeginning;

		public OverlapTask(GappedSearch<Annotation> gappedSearch, 
				Path file2,
				boolean addBeginning) {
			mGappedSearch = gappedSearch;
			mFile2 = file2;
		...
		public void done() {
			if (mNewModel != null) {
				mWindow.addToHistory("Overlap with " + PathUtils.getName(mFile2), mNewModel);
			}
		...
		private XlsxTableModel overlap() throws Exception {
			...
			if (mAddBeginning) {
				...
				cell.setCellValue("Overlap Region");
				cell = row.createCell(c++);
				cell.setCellValue("Overlap %");
				cell = row.createCell(c++);
				cell.setCellValue("Overlap Feature");
				cell = row.createCell(c++);
			...
			for (int i = 0; i < model.getRowCount(); ++i) {
				...
				if (Io.isEmptyLine(model.getText(i, 0))) {
					...
					region = null;
				} else if (GenomicRegion.isRegion(model.getText(i, 0))) {
					region = GenomicRegion.parse(model.getText(i, 0));
				} else {
					// three column format
				...
				int maxOverlapWidth = -1;
				GenomicRegion maxOverlap = null;
				...
				if (results != null) {
					for (GappedSearchFeatures<Annotation> features: results) {
						for (Annotation annotation : features) {
							...
							if (overlap != null) {
								if (overlap.getLength() > maxOverlapWidth) {
									maxOverlapWidth = overlap.getLength();
									maxOverlap = overlap;
				...
				if (mAddBeginning) {
					if (maxOverlapWidth > 0) {
						double p = 100.0 * Math.min(1.0, maxOverlapWidth / (double)region.getLength());


						cell = row.createCell(c++);
						cell.setCellValue(maxOverlap.toString());
						cell = row.createCell(c++);
						cell.setCellValue(Mathematics.round(p, 2));
						cell = row.createCell(c++);
						cell.setCellValue(maxAnnotation.getName());
						cell = row.createCell(c++);
						cell.setCellValue(maxAnnotation.getRegion().toString());
					} else {
						cell = row.createCell(c++);
						cell.setCellValue(TextUtils.NA);
						cell = row.createCell(c++);
						cell.setCellValue(TextUtils.NA);
						cell = row.createCell(c++);
						cell.setCellValue(TextUtils.NA);
	...
	private class OverlapTask extends SwingWorker<Void, Void> {
		...
		public OverlapTask(FixedGapSearch<Annotation> gappedSearch1,
				Path file,
				boolean addBeginning) {
			mGappedSearch1 = gappedSearch1;
			mFile = file;
			mAddBeginning = addBeginning;
		}
		...
		public void done() {
			if (mNewModel != null) {
				mWindow.addToHistory("Overlap with " + PathUtils.getName(mFile), mNewModel);
			}
		...
		private DataFrame overlap() throws Exception {
			...
			if (mAddBeginning) {
				...
				c = matrix.getColumnCount() - 4;
			}

			String name = PathUtils.getNameNoExt(mFile);

			matrix.setColumnName(c + 0, name + " Overlap Region");
			matrix.setColumnName(c + 1, name + " Overlap %");
			matrix.setColumnName(c + 2, name + " Overlap Feature");
			matrix.setColumnName(c + 3, name + " Overlap Location");
			...
			for (int i = 0; i < model.getRowCount(); ++i) {
				...
				if (maxOverlapWidth > 0) {
					...
					matrix.setText(i, c + 0, maxOverlap.toString());
					matrix.setValue(i, c + 1, Mathematics.round(p, 2));
					matrix.setText(i, c + 2, maxAnnotation.getName());
					matrix.setText(i, c + 3, maxAnnotation.getRegion().getLocation());
				} else {
					matrix.setText(i, c + 0, TextUtils.NA);
					matrix.setText(i, c + 1, TextUtils.NA);
					matrix.setText(i, c + 2, TextUtils.NA);
					matrix.setText(i, c + 3, TextUtils.NA);
				}
			}
	...
	private class TwoWayOverlapTask extends SwingWorker<Void, Void> {
		...
		private Map<GenomicRegion, Set<Path>> mFileMap = 
				new HashMap<GenomicRegion, Set<Path>>();
		private boolean mVenn;

		public TwoWayOverlapTask(Path file1,
				int header1,
				Path file2,
				int header2,
				boolean venn) throws InvalidFormatException, IOException, ParseException {
			mFile1 = file1;
			mFile2 = file2;
			mVenn = venn;
		...
		private DataFrame overlap() throws Exception {
			...
			Map<Chromosome, Map<GenomicRegion, Map<Path, GenomicRegion>>> overlapMap = 
					new TreeMap<Chromosome, Map<GenomicRegion, Map<Path, GenomicRegion>>>();
			...
			for (Chromosome chr1 : mGappedSearch) {
				for (Annotation annotation1 : mGappedSearch.getFeatures(chr1)) {
					...
					List<Annotation> testRegions = 
							mGappedSearch.getFeatureSet(region1);
					...
					while (!exhausted) {
						Set<GenomicRegion> groupedRegions = 
								CollectionUtils.toSet(region1);
			...
			if (mVenn) {
				SwingUtilities.invokeLater(new Runnable() {
					public void run() {
						MainVennWindow plotWindow = 
								new MainVennWindow(group1, group2, CircleStyle.PROPORTIONAL);
	...
	private class ConservationTask extends SwingWorker<Void, Void> {
		...
		public ConservationTask(boolean showMean,
				boolean showMedian,
				boolean showScores) {
			mShowMean = showMean;
	...
	private class StitchTask extends SwingWorker<Void, Void> {
		...
		public StitchTask(BinarySearch<AnnotationGene> tssSearch, 
				int distance, 
				boolean tssExclusion, 
				int ext5p, 
				int ext3p) {
		...
		private DataFrame stitch() throws IOException, ParseException {
			...
			Map<Chromosome, List<GenomicRegion>> sortedRegions =
					GenomicRegion.sortByStart(getRegions(model));
			...
			for (Chromosome chr : CollectionUtils.sort(sortedRegions.keySet())) {
				...
				while (i < regions.size()) {
					...
					for (int j = s2; j < regions.size(); ++j) {
						...
						boolean region2TssDownstream = 
								downstreamOfTss(region2, region1UpstreamGene);
		...
		private boolean inTssExZone(GenomicRegion region) {
			...
			List<AnnotationGene> closestFeatures = 
					mTssSearch.getClosestFeatures(region);
	...
	private class ExtendTask extends SwingWorker<Void, Void> {
		...
		private DataFrame extend() throws IOException, ParseException {
			...
			cell = row.createCell(c++);
			cell.setCellValue("Extended Region");

			++r;

			for (GenomicRegion region : extendedRegions) {
				row = (XSSFRow)sheet.createRow(r);

				cell = row.createCell(0);
				cell.setCellValue(region.toString());
	...
	public RegionsModule() {
		try {
			mConservationAssembly = 
					new ConservationAssemblyWeb(new URL(SettingsService.getInstance().getAsString("regions.conservation.remote-url")));
		...
		try {
			mMouseConservationAssembly = 
					new ConservationAssemblyWeb(new URL(SettingsService.getInstance().getAsString("regions.mouse.conservation.remote-url")));
		} catch (MalformedURLException e) {
			e.printStackTrace();
		} catch (IOException e) {
			e.printStackTrace();
	...
	public void init(MainMatCalcWindow window) {
		...
		button.setToolTip(new ModernToolTip("Stitch", 
				"Stitch regions together."), ribbon.getToolTipModel());
		//Ui.setSize(button, new Dimension(60, getRibbon().LARGE_BUTTON_HEIGHT));
		button.addClickListener(this);
		ribbon.getToolbar("Genomic").getSection("Regions").add(button);
		...
		button.setToolTip(new ModernToolTip("Extend", 
				"Extend regions."), ribbon.getToolTipModel());
		//Ui.setSize(button, new Dimension(60, getRibbon().LARGE_BUTTON_HEIGHT));
		button.addClickListener(this);
		ribbon.getToolbar("Genomic").getSection("Regions").add(button);
		...
		button.setToolTip(new ModernToolTip("Super Enhancers", 
				"Annotate peaks with enhancers."), ribbon.getToolTipModel());
		//Ui.setSize(button, new Dimension(60, getRibbon().LARGE_BUTTON_HEIGHT));
		button.addClickListener(this);
		ribbon.getToolbar("Genomic").getSection("Regions").add(button);
		...
		button = new RibbonLargeButton("Mouse Cons", UIService.getInstance().loadIcon("conservation", 24));
		...
		button = new RibbonLargeDropDownButton(UIService.getInstance().loadIcon("graph", 24), popup);
		...
		button.setToolTip(new ModernToolTip("Export BED Path", 
				"Export Table As BED Path."), ribbon.getToolTipModel());
		button.addClickListener(this);
	...
	private void enhancers() throws IOException {
		...
		Map<String, BinaryGapSearch<Annotation>> gappedSearch =
				dialog.getGappedSearch();

		EnhancerTask task = new EnhancerTask(gappedSearch, 
				dialog.getOverlapMode());

		task.execute();
	}

	/*
	private void overlap() throws IOException, InvalidFormatException, ParseException {
		...
		OverlapDialog dialog = new OverlapDialog(mWindow, mWindow.getInputFile(), file);
		...
		if (dialog.getStrict()) {
			OverlapTask task = new OverlapTask(gappedSearch1, 
					file, 
					dialog.getAddBeginning());

			task.execute();
		} else {
			TwoWayOverlapTask task = new TwoWayOverlapTask(mWindow.getInputFile(),
					dialog.getHeader1(),
					file,
					dialog.getHeader2(),
					dialog.getDrawVenn());
	...
	private void nWayOverlap() throws InvalidFormatException, IOException {
		...
		if (dialog.getOneWay()) {
			OneWayOverlapTask task = 
					new OneWayOverlapTask(mWindow, files, dialog.getAtBeginning(), true);

			task.doInBackground();
		} else {
			NWayOverlapTask task = new NWayOverlapTask(mWindow, 
					files, 
					dialog.getDrawVenn());
	...
	private void stitch() throws IOException {
		...
		BinarySearch<AnnotationGene> tssSearch = 
				AnnotationService.getInstance().getBinarySearch(genome);

		StitchTask task = new StitchTask(tssSearch,
				dialog.getDistance(),
				dialog.getTssExclusion(),
				dialog.getTss5p(),
				dialog.getTss3p());
	...
	private void conservationMouseOnly() throws IOException, InvalidFormatException {
		...
		ConservationMouseOnlyTask task = 
				new ConservationMouseOnlyTask(dialog.getShowConservation(),
						dialog.getShowScores());
	...
	private void tssPlot() throws IOException, ParseException  {
		...
		TssPlotTask task = new TssPlotTask(mWindow,
				tssSearch,
				dialog.getStart(),
				dialog.getEnd(),
				dialog.getUnits(),
	...
	private void distancePlot() throws IOException, InvalidFormatException {
		...
		DistancePlotTask task = new DistancePlotTask(mWindow,
				...
				plotDialog.getEnd(),
				plotDialog.getUnits(),
				plotDialog.getBinSize(),
				plotDialog.getBinUnits());
