...
public class NWayOverlapDialog extends ModernDialogHelpWindow implements ModernClickListener {
	...
	private ModernRadioButton mCheckTwoWay = 
			new ModernRadioButton("Two way");
	
	private ModernTwoStateWidget mCheckVenn = 
			new ModernCheckSwitch("Create Venn diagram");
	
	private ModernTwoStateWidget mCheckBeginning = 
			new ModernCheckSwitch("At beginning", true);
	...
	public NWayOverlapDialog(MainMatCalcWindow parent) {
		...
		mChooseFilesPanel = new ChooseFilesPanel(parent, new AllRegionGuiFileFilter(),
				BedGuiFileFilter.INSTANCE,
				BedGraphGuiFileFilter.INSTANCE,
				CsvGuiFileFilter.INSTANCE,
	...
	public void clicked(ModernClickEvent e) {
		if (e.getSource().equals(mOkButton)) {
			...
			SettingsService.getInstance().update("org.matcalc.toolbox.bio.regions.nway.one-way",
